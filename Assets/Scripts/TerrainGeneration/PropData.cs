...
{
	[Serializable]
	public class PropData : IProcessPropPointData
	{
		...
		protected Vector3 CalculatePosition(Vector3 position, MapData mapData, float factor = 10)
		{
			position.y = mapData._heightMultiplier;

			if (!Physics.Raycast(position, Vector3.down, out RaycastHit hit, mapData._heightMultiplier + factor,
				    LayerMask.GetMask(mapData._terrainLayer))) return Vector3.positiveInfinity;

			position.y = hit.point.y - DropIntoTerrainAmount;
