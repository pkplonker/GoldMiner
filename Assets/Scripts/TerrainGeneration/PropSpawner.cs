...
namespace TerrainGeneration
{
	public class PropSpawner : MonoBehaviour
	{
		...
		public void SpawnObjects(ChunkManager chunkManager)
		{
			...
			OnPropsGenerationStarted?.Invoke(PropCollections.Props.Count);
			StartCoroutine(SpawnObjectsCor(chunkManager.MapData));
		}
		...
		private IEnumerator SpawnObjectsCor(MarchingCubeMapData mapData)
		{
			...
			for (var j = 0; j < PropCollections.Props.Count; j++)
			{
				...
				var task = Task.Run(() => PoissonDiscSampling.GeneratePointsCor(index: j1, maxPointsPerProp,
					new Vector2(spawnArea, spawnArea), PoissonCallback, chunkManager.MapData,
			...
			while (index != numberOfDifferentPropsToSpawn)
			{
				...
				StartCoroutine(PropCollections.Props[data.Index].ProcessPointDataCor(data,
					PropSpawnCompleteCallback, this, chunkManager.MapData));
				yield return null;
			}

			for (var i = 0; i < chunkManager.maxChunkCoord.x; i++)
			{
